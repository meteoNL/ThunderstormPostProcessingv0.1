...
qrf_procedure <- function(train_set, test_set, predictant_index, varindexset, m_hyp, ntree_hyp, node_size_hyp, min_length_setting, wval, yval){
  ...
  while (length(train_set[varindexset])>min_length_setting){
    ...
    write.csv(list(qrf_fit$importance,m_hyp,ntree_hyp,node_size_hyp,wval,yval), "fit_importance.csv")
    ...
    write.csv(predict_dataset, "predicted_quantiles.csv")
    ...
    write.csv(df_with_crpsscore,"last_CRPS_calc.csv")
