...
qrf_procedure <- function(train_set, test_set, predictant_index, varindexset, m_hyp, ntree_hyp, node_size_hyp, min_length_setting, wval, yval){
  ...
  while (length(train_set[varindexset])>min_length_setting){
    ...
    qrf_pred = data.frame(prob = fit1_pred$predictions, occurence = test_set[predictant_index], region = test_set["region"], npred = (length(varindexset)+1), mtry = m_hyp, effmtry = min(m_hyp, (length(varindexset))), min_n_size = node_size_hyp, w = wval, y = yval)
